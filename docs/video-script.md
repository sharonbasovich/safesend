...
| 1:10–1:35 | Pending → QUARANTINED card → Cancel & refund → settled | "On-chain, the escrow is quarantined. Only the intended recipient can claim — and for a flagged lookalike, even that requires the victim's explicit on-chain approval, which never happens automatically. The victim cancels, and gets a full refund. That same paste into a raw transfer is gone forever; through SafeSend it's a timeout and a refund." |
...
| 2:30–2:45 | Slide: limitations | "No owner, no fees, no upgradeability, checks-effects-interactions, fifty-seven tests including fuzz and invariants at full line coverage. Honest limits: it only guards sends through the router, fingerprints only cover the first and last four characters, and it's an unaudited prototype. But the most common way people lose money to poisoning just became a cancellable escrow. SafeSend." |
