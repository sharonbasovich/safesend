...
| 1:10–1:35 | Pending → QUARANTINED card → Cancel & refund → settled | "On-chain, the escrow is quarantined. Only the address named as recipient can claim — and for a flagged lookalike, even that requires the sender's explicit on-chain approval, which never happens automatically. The sender cancels while pending and gets a full refund. That same paste into a raw transfer is gone forever; through SafeSend it's a chance to inspect and recover." |
...
| 2:30–2:45 | Slide: limitations | "No owner, no fees, no upgradeability, checks-effects-interactions, fifty-eight tests including fuzz and invariants at full line coverage. Honest limits: it only guards sends through the router, fingerprints only cover the first and last four characters, and it's an unaudited prototype. Flagged recipients need the sender's explicit approval before claiming. SafeSend." |
