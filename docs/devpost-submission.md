...
- **Lookalike** — any address whose fingerprint collides with a verified
  ...
  cancel while pending. If the approved recipient claims, their full address
  becomes verified for future instant sends; the sender must check it carefully.
...
- **Testing:** Forge unit/negative/fuzz (512 runs) + invariant suite
  (128 runs × depth 32); 58 tests, 100% line coverage on SafeSend.sol
...
- **Does not prevent every loss.** It protects sends routed through
  SafeSend; raw transfers and lookalikes that match a different visible-address
  pattern (for example, the first six characters but not the last four) are
  out of scope. Verification via `addPayee` is only as good as the sender's
  out-of-band check.
